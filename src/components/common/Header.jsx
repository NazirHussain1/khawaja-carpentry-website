...
function TopBarLink({ href, icon: Icon, children, className = '' }) {
  return (
    <a
      className={`inline-flex items-center gap-1.5 whitespace-nowrap text-xs font-medium text-white/95 transition hover:-translate-y-0.5 hover:text-white ${className}`}
      href={href}
  ...
  return (
    <header className="sticky top-0 z-[100] bg-[#02024f] shadow-2xl shadow-slate-950/30">
      <div className="bg-gradient-to-r from-violet-700 via-indigo-600 to-sky-600">
        <div className="mx-auto flex min-h-9 max-w-7xl flex-wrap items-center justify-center gap-x-5 gap-y-1 px-4 py-2 sm:px-6 lg:justify-between lg:px-8 lg:py-0">
          <div className="flex flex-wrap items-center justify-center gap-x-5 gap-y-1 lg:justify-start">
            <TopBarLink href={`mailto:${contact.email}`} icon={Mail}>{contact.email}</TopBarLink>
